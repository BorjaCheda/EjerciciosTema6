...
public class EmpleadoProyectoController {
    ...
    @GetMapping("/delete/{idE}/{idP}")
    public String showDeleteEmpl(@PathVariable long idE, @PathVariable long idP) {
        Empleado e = empleadoService.obtenerPorId(idE);
        Proyecto p = proyectoService.obtenerPorId(idP);
        EmpleadoProyecto ep = empleadoProyectoService.obtenerPorEmpleadoProyecto(e, p);
        empleadoProyectoService.borrar(ep);
        return "redirect:/";
    }
    @GetMapping("/new")
